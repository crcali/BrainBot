    ...
    {*/
      for ( i=0; i<7; i++) {
        ...
        while  (SerSerial.available() == 0) {
          ...
          //     SerSerial.print("turning off\n");
          //     SerTurnRobotOff();
          //   }*/
          //   return;  // 
          //}      
        } 
        // char recieved = SerSerial.read();
        // inData += recieved; 
      ...
      SerSerial.println(abDualShock[5], HEX);
      SerSerial.println(abDualShock[6], HEX);
  // Lets check the checksum...  MC--made != should be ==
  ...
  if (checkSumTrue) {
    ...
    if (ButtonPressed(SERB_START)) {// OK lets try "0" button for Start. 
      ...
      if (g_InControlState.fRobotOn) {
        ...
        SerSerial.print("Started2");
        fAdjustLegPositions = true;
      }
    }
    SerSerial.print("froboton");
    ...
    if (g_InControlState.fRobotOn) {
      ...
      if (ButtonPressed(SERB_SQUARE)) { // Square Button Test
        SerSerial.print("SWITCHBALANCEMODE");
      ...
      if (ButtonPressed(SERB_TRIANGLE)) { // Triangle - Button Test
        SerSerial.print("SERB_TRIANGLE- STANDUP/SITDOWN");
        if (g_BodyYOffset>0) 
          g_BodyYOffset = 0;
        else
          g_BodyYOffset = 35;
      ...
      if (ButtonPressed(SERB_PAD_UP)) {// D-Up - Button Test
        SerSerial.print("D-UP- STANDUP/SITDOWN");
        g_BodyYOffset += 10;
      ...
      if (ButtonPressed(SERB_PAD_DOWN) && g_BodyYOffset) {// D-Down - Button Test
        SerSerial.print("D-Down- STANDUP/SITDOWN");
        if (g_BodyYOffset > 10)
          g_BodyYOffset -= 10;
        else
      ...
      if (ButtonPressed(SERB_PAD_RIGHT)) { // D-Right - Button Test
        SerSerial.print("D-Right- STANDUP/SITDOWN");
      ...
      if (ButtonPressed(SERB_PAD_LEFT)) { // D-Left - Button Test
        SerSerial.print("D-Left- STANDUP/SITDOWN");
      ...
      if (ControlMode == WALKMODE) {
        ...
        && abs(g_InControlState.TravelLength.z)<cTravelDeadZone 
          ...
          SerSerial.print("CHANGEGAIT");
          GaitSelect();
        }

        //Double leg lift height
        if (ButtonPressed(SERB_R1)) { // R1 Button Test
          SerSerial.print("DoubleLegOn");
          MSound( 1, 50, 2000); 
        ...
        if (ButtonPressed(SERB_R2)) {// R2 Button Test
          SerSerial.print("DoubleTravelOn");
          MSound(1, 50, 2000); 
