...
def moveForward(speed, distance):
	...
	sp.write(("#0 P2000 s%i\r" %speed).encode())
	sp.write(("#25 P2000 s%i\r" %speed).encode())
	sp.write(("#24 P1300 s%i\r" %speed).encode())
	time.sleep(2)
	sp.write(("#1 P2150 s%i\r" %speed).encode())
	sp.write(("25 P2215 s%i\r" %speed).encode())
	time.sleep(5)
	sp.write(("#17 P2600 s%i\r" %speed).encode())
	sp.write(("#16 P500 s%i\r" %speed).encode())
	sp.write(("#9 P2500 s%i\r" %speed).encode())
	sp.write(("#8 P1800 s%i\r" %speed).encode())
	time.sleep(2)
	sp.write(("#17 P2425 s%i\r" %speed).encode())
	sp.write(("#9 P2300 s%i\r" %speed).encode())
