...
<speed (must be between 100 and 2000)> 
<distance (must be a factor of five) | angle (must be a factor of 45)>
...
class RobotCommands(threading.Thread):
    def __init__(self, INPUT, SPEED, DISTANCE):
    ...
    def run(self):
        ...
        while not self.shutdown_flag.is_set():
            ...
            if self.command == 'forward':
                ...
                defultPosition()
                numberOfTimes = (int(self.distance)/5)*2
                ...
                for x in range (0,numberOfTimes):
                    ...
                    moveServo(8, 1900, self.speed, waitTime)
            ...
            if self.command == 'backward':
                ...
                for x in range (0,numberOfTimes):
                    ...
                    #lifts up second leg 
                    moveServo(1, 1535, self.speed, waitTime)
                    moveServo(0, 950, self.speed, waitTime)
                    moveServo(1, 2150, self.speed, waitTime)
                    ...
                    #lifts up forth leg 
                    moveServo(9, 2350, self.speed, waitTime)
                    moveServo(8, 900, self.speed, waitTime)
                    moveServo(9, 1460, self.speed, waitTime)
            ...
            if self.command == 'turn':
                ...
                self.speed = int(self.speed)
                self.distance = int(self.distance)
                numberOfTimes = (int(self.distance)/45)
                waitTime = (int(self.speed/1000))+.25

                for x in range (0,numberOfTimes):
                    #lifts up first leg 
                    moveServo(25, 1600, self.speed, waitTime)
                    moveServo(24, 2700, self.speed, waitTime)
                    if not self.shutdown_flag.is_set(): sp.write(("#25 P2215 #26 P1450 T%i\r" %self.speed).encode())
                    if not self.shutdown_flag.is_set(): time.sleep(waitTime)

                    #lifts up second leg 
                    moveServo(1, 1535, self.speed, waitTime)
                    moveServo(0, 2300, self.speed, waitTime)
                    moveServo(1, 2150, self.speed, waitTime)

                    #lifts up third leg 
                    moveServo(17, 2500, self.speed, waitTime)
                    moveServo(16, 2300, self.speed, waitTime)
                    moveServo(17, 1600, self.speed, waitTime)

                    #lifts up forth leg 
                    moveServo(9, 2350, self.speed, waitTime)
                    moveServo(8, 2350, self.speed, waitTime)
                    moveServo(9, 1460, self.speed, waitTime)
                    ...
                    #lifts up first leg 
                    moveServo(25, 1600, self.speed, waitTime)
                    moveServo(24, 1650, self.speed, waitTime)
                    moveServo(25, 2215, self.speed, waitTime)

                    #lifts up second leg 
                    moveServo(1, 1535, self.speed, waitTime)
                    moveServo(0, 1475, self.speed, waitTime)
                    moveServo(1, 2150, self.speed, waitTime)

                    #lifts up third leg 
                    moveServo(17, 2500, self.speed, waitTime)
                    moveServo(16, 1650, self.speed, waitTime)
                    moveServo(17, 1600, self.speed, waitTime)

                    #lifts up forth leg 
                    moveServo(9, 2350, self.speed, waitTime)
                    moveServo(8, 1650, self.speed, waitTime)
                    moveServo(9, 1460, self.speed, waitTime)
